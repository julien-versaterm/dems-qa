...
- **F9:** A successful external share is **not shown in the file-level CoC — though the share IS functional and IS audited** (not a "share is broken" problem). Root cause pinned on 2026-07-06 (UI + API): the `share` audit row records the shared file under **`reference_data.resource_ids`**, but the CoC file-trail query matches on **`reference_data.file_ids`** JSONB containment (`audit-service/app/datastore/daos/audit_log_dao.py:142`) and the generator reads `reference_data["file_ids"]` (`chain_of_custody_generator.py:117`). Every other file event (`view`/`download`/`upload`) writes `file_ids` and appears; `share` writes `resource_ids` and is missed. Evidence: `GET /audit/logs?q=share` returns the `share files 201` row (with `resource_ids`, `expiry`, requester, `success`); `GET /audit/logs?file_id=<fid>` for the same file omits it. **Fix is small:** have the share emitter also populate `reference_data.file_ids`. Also observed: the external **share endpoint requires an `expiry`** field (400 without it), and a file **cannot be shared once privatized** ("1 file failed to share") — share must precede any restrict.
